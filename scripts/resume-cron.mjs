/**
 * One-off script: resume (unpause) a cron job by thread name.
 * Unarchiving the forum thread triggers forum-sync to re-enable the scheduler job.
 *
...
client.once('ready', async () => {
  try {
    ...
    const activeThread = activeThreads.find((t) => t.name === name);

    if (activeThread) {
      console.log(`"${name}" is already active (not paused). threadId=${activeThread.id}`);
      return;
    }
