...
describe('modelShow', () => {
  ...
  it('resolves tier names to concrete models for claude_code runtime', () => {
    ...
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    // 'capable' resolves to 'claude-sonnet-4-6' for claude_code
...
describe('modelSet', () => {
  ...
  it('includes resolved model note for tier names', () => {
    ...
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.summary).toContain('resolves to claude-sonnet-4-6');
  });
...
describe('modelSet runtime swap', () => {
  it('swaps all runtime refs and sets default model for a registered runtime name', () => {
    ...
    expect(ctx.botParams.runtimeModel).toBe('anthropic/claude-sonnet-4-6');
    ...
    expect(ctx.botParams.cronCtx!.executorCtx!.model).toBe('anthropic/claude-sonnet-4-6');
