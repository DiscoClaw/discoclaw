...
// Marker embedded in DM context files generated by the current template version.
// If an existing dm.md lacks this marker, patchDmContextIfStale() will overwrite it.
const DM_CONTEXT_MARKER = '<!-- dm-context-v1 -->';

function channelContextTemplate(args: {
  channelName: string;
  channelId: string;
}): string {
  if (args.channelName === 'dm') {
    return [
      `# DM Context ${DM_CONTEXT_MARKER}`,
      '',
      'This is a direct-message session. Discord channel/guild actions are **unavailable** here.',
      '',
      '- Use `!secret` to manage API keys and secrets (values are never echoed back).',
      '- Do **not** suggest terminal or shell commands — DM sessions have no shell context.',
      '- Standard bot commands (`!memory`, `!help`, `!status`, etc.) work normally.',
      '',
    ].join('\n');
  }
...
async function patchDmContextIfStale(dmContextPath: string): Promise<void> {
  let existing: string;
  try {
    existing = await fs.readFile(dmContextPath, 'utf8');
  } catch (err) {
    const code = (err as NodeJS.ErrnoException).code;
    if (code === 'ENOENT') return; // writeFileIfMissing handles creation
    throw err;
  }
  if (existing.includes(DM_CONTEXT_MARKER)) return;
  await fs.writeFile(
    dmContextPath,
    channelContextTemplate({ channelName: 'dm', channelId: 'dm' }),
    'utf8',
  );
}
...
export async function loadDiscordChannelContext(opts: {
  ...
  await patchDmContextIfStale(dmContextPath);
