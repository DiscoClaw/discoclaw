...
export type ImagegenActionRequest =
  | { type: 'generateImage'; prompt: string; channel: string; size?: string; model?: string; quality?: string; caption?: string };
...
const VALID_SIZES = new Set(['256x256', '512x512', '1024x1024', '1024x1792', '1792x1024']);
...
export async function executeImagegenAction(
  ...
  switch (action.type) {
    case 'generateImage': {
      ...
      const size = action.size ?? DEFAULT_SIZE;
      if (!VALID_SIZES.has(size)) {
        return { ok: false, error: `Invalid size "${size}". Allowed: ${[...VALID_SIZES].join(', ')}` };
      }
      ...
      try {
        response = await fetch(`${baseUrl}/images/generations`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${imagegenCtx.apiKey}`,
      ...
      const buf = Buffer.from(imageItem.b64_json, 'base64');
