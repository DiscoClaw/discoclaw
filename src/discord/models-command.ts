...
export function handleModelsCommand(cmd: ModelsCommand, opts: ModelsCommandOpts): string {
  ...
  if (cmd.action === 'help') {
    return [
      ...
      '',
      '**Canonical runtime names:** `claude-cli`, `claude-api`, `codex-cli`, `gemini-api`, `openai`, `openrouter`',
      'Compatibility aliases: `claude` → `claude-cli`, `anthropic` → `claude-api`, `codex` → `codex-cli`.',
      '`claude-api` is voice-only. Use it with `!models set voice claude-api`; `chat` cannot switch to `claude-api`.',
      '',
      ...
      'The `-cli` suffix denotes a local subprocess runtime; `-api` or plain name denotes an HTTP API runtime.',
      '',
      ...
      '- `!models set voice claude-api` — switch voice to the Claude API voice-only runtime',
