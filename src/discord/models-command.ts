...
export function handleModelsCommand(cmd: ModelsCommand, opts: ModelsCommandOpts): string {
  ...
  if (cmd.action === 'help') {
    return [
      ...
      '- `!models reset` — revert all roles to startup defaults',
      ...
      'Setting the `chat` or `voice` role to a runtime name (`openrouter`, `openai`, `gemini-api`, `gemini-cli`, `codex`, `claude`) switches the active runtime adapter so invocations route through that provider.',
      'For Gemini runtime swaps, `gemini-api` and `gemini-cli` are the explicit runtime paths. The legacy `gemini` name remains a compatibility alias for `gemini-api`.',
