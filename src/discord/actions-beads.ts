...
export async function executeBeadAction(
  ...
  switch (action.type) {
    case 'beadCreate': {
      ...
      const bead = beadCtx.store.create({
        title: action.title,
      ...
      if (beadCtx.autoTag && tagNames.length > 0) {
        try {
          ...
          for (const tag of suggestedTags) {
            try { beadCtx.store.addLabel(bead.id, `tag:${tag}`); } catch {}
          }
        } catch (err) {
      ...
      try {
        ...
        if (labels.includes('no-thread') || (bead.labels ?? []).includes('no-thread')) {
          ...
          try {
            beadCtx.store.update(bead.id, { externalRef: `discord:${threadId}` });
          } catch (err) {
    ...
    case 'beadUpdate': {
      ...
      const bead = beadCtx.store.update(
        action.beadId,
        {
          title: action.title,
          description: action.description,
    ...
    case 'beadClose': {
      ...
      const bead = beadCtx.store.close(action.beadId, action.reason);
    ...
    case 'beadShow': {
      ...
      const bead = beadCtx.store.get(action.beadId);
      if (!bead) {
    ...
    case 'beadList': {
      ...
      const beads = beadCtx.store.list({
