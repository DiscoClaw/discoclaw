...
function formatTaskSyncVerboseLines(counters: Record<string, number>): string[] {
  ...
  lines.push(
    `Task sync transitions: created=${asCount(counters, 'tasks.sync.transition.threads_created')} renamed=${asCount(counters, 'tasks.sync.transition.thread_names_updated')} starter=${asCount(counters, 'tasks.sync.transition.starter_messages_updated')} statuses=${asCount(counters, 'tasks.sync.transition.statuses_updated')} tags=${asCount(counters, 'tasks.sync.transition.tags_updated')} archived=${asCount(counters, 'tasks.sync.transition.threads_archived')} reconciled=${asCount(counters, 'tasks.sync.transition.threads_reconciled')} orphans=${asCount(counters, 'tasks.sync.transition.orphan_threads_found')} deferred=${asCount(counters, 'tasks.sync.transition.closes_deferred')} warnings=${asCount(counters, 'tasks.sync.transition.warnings')}`,
  );
  lines.push(
