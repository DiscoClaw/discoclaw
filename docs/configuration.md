...
## Setup Boundary: Source Checkout vs npm-Managed vs Claude Auth
...
2. **Config-only doctor surfaces shared by both install modes** are handled by [`inspect()`](/home/davidmarsh/code/discoclaw/src/health/config-doctor.ts#L667) and [`applyFixes()`](/home/davidmarsh/code/discoclaw/src/health/config-doctor.ts#L747) in [`src/health/config-doctor.ts`](/home/davidmarsh/code/discoclaw/src/health/config-doctor.ts). For the npm-managed CLI path, [`discoclaw doctor`](/home/davidmarsh/code/discoclaw/src/cli/index.ts#L357) loads `.env` and then calls those functions from [`src/cli/index.ts`](/home/davidmarsh/code/discoclaw/src/cli/index.ts). In Discord, [`!doctor` and `!health doctor`](/home/davidmarsh/code/discoclaw/src/discord/message-coordinator.ts#L1137) route through the same config-doctor module in [`src/discord/message-coordinator.ts`](/home/davidmarsh/code/discoclaw/src/discord/message-coordinator.ts). Those handlers inspect local config, runtime/model state files, and workspace bootstrap state only; they do not invoke Claude.
...
4. **Claude auth validation** remains separate by install mode. Source checkouts use [`scripts/claude-auth-smoke.ts`](/home/davidmarsh/code/discoclaw/scripts/claude-auth-smoke.ts) via `pnpm claude:auth-smoke`. npm-managed installs use [`discoclaw claude auth-smoke`](/home/davidmarsh/code/discoclaw/src/cli/index.ts#L315) from [`src/cli/index.ts`](/home/davidmarsh/code/discoclaw/src/cli/index.ts).

That boundary is intentional. The checked-in audit memos at [docs/audit/claude-blank-machine-readiness.md](/home/davidmarsh/code/discoclaw/docs/audit/claude-blank-machine-readiness.md) and [docs/audit/claude-npm-managed-path.md](/home/davidmarsh/code/discoclaw/docs/audit/claude-npm-managed-path.md) are the authoritative 1.0 readiness verdicts for the source-checkout and npm-managed Claude paths respectively. Treat a passing `pnpm preflight:blank-machine`, `pnpm preflight`, `discoclaw doctor`, `!doctor`, or `!health doctor` run as evidence for config/bootstrap prerequisites only. Treat the Claude auth smoke result separately as the install-mode-specific Claude login/auth check.

Source-only auth and preflight scripts are unavailable in npm installs because they are not shipped in [`package.json`](/home/davidmarsh/code/discoclaw/package.json). The published `package.json.files` list includes `dist/`, selected docs, templates, and assets, but not repo-owned source helpers such as `scripts/doctor.ts` or `scripts/claude-auth-smoke.ts`.

For npm-managed daemon installs, readiness is currently constrained by service executable and `PATH` resolution, not just by manual Claude login. [`src/cli/daemon-installer.ts`](/home/davidmarsh/code/discoclaw/src/cli/daemon-installer.ts) hardcodes `/usr/bin/node` in both service renderers and gives the service a fixed `PATH`, while [`discoclaw init`](/home/davidmarsh/code/discoclaw/src/cli/init-wizard.ts) does not persist `CLAUDE_BIN`. A passing `discoclaw claude auth-smoke` therefore proves the interactive shell path only; it does not yet prove daemon parity with that shell.
