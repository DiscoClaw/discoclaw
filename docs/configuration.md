...
| `CODEX_APP_SERVER_NATIVE` | `false` | When `1` and `CODEX_APP_SERVER_URL` is set, use the app-server WebSocket as the primary invoke transport instead of `codex exec`. Automatically falls back to CLI for image turns, non-default `cwd`, non-empty `addDirs`, and connection failures. |
