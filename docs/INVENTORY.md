...
| OpenRouter env-key path and audited proof boundary (OpenAI-compat adapter at `openrouter.ai/api/v1`, `id: 'openrouter'`, with shipped `GET /models` credential proof and `openrouter-key: ok` live evidence; source checkouts add repo smoke-path workload coverage, npm-managed `doctor` remains config-only, and tier defaults stay manual/deferred, see [OpenRouter env-key parity checklist](configuration.md#openrouter-env-key-parity-checklist)) | `src/runtime/openai-compat.ts`, `src/health/credential-check.ts`, `src/discord/status-command.ts`, `src/runtime/model-smoke.test.ts` | **done** |
