...
| Tasks (task tracking) | create, update, close, show, list, sync | `task-action-contract.ts`, `task-action-executor.ts`, `task-action-prompt.ts` | **done** |
...
| Sync apply/reconcile phase executors | `src/tasks/task-sync-apply.ts` | **done** |
