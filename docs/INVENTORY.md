...
| OpenRouter env-key path, built-in tier defaults, and audited proof boundary (shared OpenAI-compatible adapter at `openrouter.ai/api/v1`, `id: 'openrouter'`, with optional OpenRouter-only `provider` payload forwarding, shipped `fast` / `capable` / `deep` defaults, and `GET /models` credential proof surfaced as `openrouter-key: ok`; source checkouts add repo smoke-path workload coverage through the dedicated `OPENROUTER_SMOKE_TEST_TIERS` suite, while npm-managed `doctor` remains config-only, see [OpenRouter env-key parity checklist](configuration.md#openrouter-env-key-parity-checklist)) | `src/runtime/openai-compat.ts`, `src/runtime/model-tiers.ts`, `src/config.ts`, `src/index.runtime.ts`, `src/health/credential-check.ts`, `src/discord/status-command.ts`, `src/runtime/openrouter-smoke.test.ts` | **done** |
...
| Messaging | send, edit, delete, react, pin, fetch, sendFile, downloadAttachment | `actions-messaging.ts` | **done** |
