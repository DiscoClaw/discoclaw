...
1. Refuse to run unless the current tree is a real source checkout, `<repoRoot>/.env` exists, and that repo-local `.env` resolves both `PRIMARY_RUNTIME=claude-cli` and a concrete `DISCORD_GUILD_ID`.
