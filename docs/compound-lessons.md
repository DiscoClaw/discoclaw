...
Applied: `src/observability/trace-store.ts`, `src/observability/metrics.ts`, `src/discord/message-coordinator.ts`, `src/discord/reaction-handler.ts`, `src/cron/executor.ts`, `src/dashboard/api/traces.ts`, `src/dashboard/api/metrics.ts`
Status: active

### 2026-04-05 - Wire WebSocket session resilience as TTL-tracked resume, lifecycle events, and terminal teardown
Tags: #voice #architecture #workflow
Lesson: Long-lived bidirectional WebSocket sessions (e.g. Gemini Live) need three cooperating resilience layers, not just retry loops: (1) **resume handle TTL tracking** — store the server-issued resume handle with a timestamp and expire it locally *before* the server-side TTL to avoid racing the server expiry (Gemini: 90s local vs ~120s server); (2) **typed reconnect lifecycle events** — emit discrete events (`reconnecting`, `reconnected`, `reconnect_failed`) with diagnostic metadata (`attempt`, `hasResumeHandle`) so downstream consumers can pause playback, log state, and distinguish session-resuming reconnects from fresh-session reconnects; (3) **terminal failure teardown wiring** — when retries are exhausted, propagate failure through a callback (`onSessionTerminated`) to the pipeline manager so it tears down the full resource tree (provider, responder, audio receiver) instead of leaving orphaned components. The resume handle and lifecycle events belong in the provider; the teardown callback belongs in the responder-to-pipeline boundary. Separating these layers keeps each testable in isolation while ensuring the full chain fires correctly end to end.
Source: task/chat context — Gemini Live voice sessions needed to survive transient WebSocket drops; the provider already stored the resume handle and included it on reconnect but lacked TTL expiry, lifecycle event emission, and terminal failure propagation to the pipeline manager
Applied: `src/voice/providers/gemini-live-provider.ts`, `src/voice/providers/gemini-live-types.ts`, `src/voice/providers/gemini-live-responder.ts`, `src/voice/audio-pipeline.ts`, `docs/voice.md`, `docs/compound-lessons.md`
Status: active
