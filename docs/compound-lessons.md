...
### 2026-03-24 - Design interactive verification steps as injectable dependencies for automation
Tags: #workflow #architecture #task
Lesson: When a test harness or rehearsal script includes manual interactive checkpoints (TTY/readline prompts), design them behind an injectable dependency so the entire orchestration flow can be automated without rewriting it. The `promptCheckpoint` dependency pattern — where a function signature accepts a checkpoint prompt and returns a status — lets the same rehearsal flow swap between a human-at-terminal readline implementation and a programmatic Discord API probe-and-poll implementation via a flag (`--auto`). The automated path must own its own lifecycle (observer client login, channel resolution, dispose/cleanup) and map each checkpoint ID to a specific verification strategy rather than applying a single generic check, because different checkpoints exercise materially different system properties (message handling, context continuity, task sync, cron execution, restart recovery).
Source: plan-629 auto-checkpoint implementation — 5 manual TTY/readline checkpoints in `scripts/release-rehearsal.ts` replaced with programmatic Discord API verification behind `--auto` flag; dedup search on 2026-03-24 found no existing lesson covering injectable verification dependencies or TTY-to-API checkpoint automation
Applied: `scripts/auto-checkpoint.ts`, `scripts/release-rehearsal.ts`, `scripts/__tests__/auto-checkpoint.test.ts`
Status: active
