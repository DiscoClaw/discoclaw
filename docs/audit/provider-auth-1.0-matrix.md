...
| --- | --- | --- | --- | --- | --- |
| Claude CLI / OAuth | Source checkout | `SUPPORTED FOR 1.0` | Repo-owned Claude path is support-claimable when `pnpm preflight*` stays config-only and `pnpm claude:auth-smoke` passes after interactive Claude login. | Current contract is narrow rather than one-command automated. The 2026-03-22 `/tmp/discoclaw-test` audit proved the isolated pre-login failure and a separate logged-in-shell success, but it did not perform interactive login in that same no-session shell, so the first-login stranger release gate remains separately open. | `docs/audit/claude-blank-machine-readiness.md` |
