...
- `Claude CLI` auth on a source checkout is the intended default path, but it remains `PARTIAL` until the same-shell first-login evidence below is captured.
...
| --- | --- | --- | --- | --- | --- |
| Claude CLI / OAuth | Source checkout | `PARTIAL` | Repo-owned Claude path is support-claimable today only for the narrower `fresh-clone post-login path proven` slice. Upgrade this row to `SUPPORTED FOR 1.0` only after `pnpm preflight*` stays config-only and the same no-session shell or account records the pre-login unauthenticated result, interactive `claude` login, and post-login `pnpm claude:auth-smoke` success rerun. | The 2026-03-22 `/tmp/discoclaw-test` audit proved the isolated pre-login failure and a separate logged-in-shell success, but it did not perform interactive login in that same no-session shell, so the first-login stranger release gate remains open. | `docs/audit/claude-blank-machine-readiness.md`, `docs/audit/claude-blank-machine-path.md` |
